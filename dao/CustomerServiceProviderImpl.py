...
class CustomerServiceProviderImpl(ICustomerServiceProvider):
    ...
    def create_account(self, customer: Customer, acc_no, accType, balance):
        ...
        if accType == "savings":
            result = stmt.execute(f"insert into accounts (customer_id,account_type,balance) VALUES ( {customer.get_customer_info()['customer_id']}, 'savings', {balance});")
            print(result)
        else:
            result = stmt.execute(
    ...
    def withdraw(self, account: Account, amount):
        ...
        stmt = self.__conn.cursor()
        stmt.execute(
            f"insert into transactions (account_id,transaction_type,amount,transaction_date) VALUES ( {account.get_account_info()['account_id'][0]}, 'withdrawal', {amount}, '{datetime.date.today()}');")
        stmt.execute(
            f"update accounts set balance = balance - {amount} where account_id = {account.get_account_info()['account_id'][0]};")
        print(self.get_account_balance(account.get_account_info()["account_id"][0]))
        # stmt.commit()
